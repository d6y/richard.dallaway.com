...
A paper from June this year showed how to use Cartesian Genetic Programming to play Atari 2600 games. What I find exciting about this is that the system evolves _programs_.  That's powerful while also producing transparent results compared to other approaches. This post explores the details of the representation used in the paper.
...
What you're looking at is a graph of computations. At the top are the inputs, representing the red,  green and blue channels of a video frame.
This particular program doesn't make use of the third (blue) input.
...
For example, output 5 receives a value from the `atan` function (arctangent). That, in turn, takes input from a `first` and `add` function. What the functions do doesn't matter much, but note that they are the fundamental building blocks provided in the experiment. That is, the evolution process isn't inventing `atan`, we're giving it `atan` as one of the functions it can use.
...
As it turns out, the above program in the video presses "left and fire" most of the time, although it is already as far left as it can go.
...
That's one algorithm (a "deep Q-network"), but as we've seen, other algorithms are available. The above work, published in the Genetic and Evolutionary Computation Conference, shows a CGP alternative, with competitive performance. But notice that the solution evolved is something we inspect.

I find all of this interesting for a couple of reasons: 

- the rich representation you can use as building blocks; and
- simple programmes might be things we can understand.
